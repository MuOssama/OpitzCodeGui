...
def Digit1st():
    ...
    def calculate_code():
        if var.get() == 1:  # Rotational selected
            ...
            # For example:
            code = int((A + B + C) * 100)  # Convert to integer for a 5-digit code
            code_label.config(text=f"Code: {code:05d}")
...
def Digit5th():
    ...
    Radiobutton(case_2_buttons, text="Bevel Gear", variable=selected_digit_case_2, value=6).pack(anchor=CENTER)
    Radiobutton(case_2_buttons, text="Internal Spline", variable=selected_digit_case_2, value=7).pack(anchor=CENTER)
...
def finish():
    ...
    FinishButton = Button(root, text='Finish',command=root.destroy ,width=25,height=3, bg='red')
